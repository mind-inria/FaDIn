...
def set_data(end_time, n_discrete, random_state):
    ...
    return events, baseline, alpha, decay, discrete
...
def test_squared_term_l2loss():
    ...
    events, baseline, alpha, decay, discrete = set_data(end_time, n_discrete,
                                                        random_state)

    model = DiscreteKernelFiniteSupport(0, 1, delta, kernel='Exponential', n_dim=2)
    kernel = model.eval([decay], discrete)
    intens = model.intensity_eval(baseline, alpha, [decay], events, discrete)
    squared_conv = 2 * ((intens**2).sum(1) * 0.5 * delta).sum()

    zG = get_zG(events.numpy(), n_discrete)
    ztzG = get_ztzG(events.numpy(), n_discrete)

    term_1 = end_time * term1(baseline)
    term_2 = 2 * delta * term2(torch.tensor(zG), baseline, alpha, kernel)
    term_3 = delta * term3(torch.tensor(ztzG), alpha, kernel)

    squared_precomp = term_1 + term_2 + term_3
...
def test_right_term_l2loss():
    ...
    kernel = model.eval([decay], discrete)
    intens = model.intensity_eval(baseline, alpha, [decay], events, discrete)
    right_term_conv = 2 * (intens * events).sum()

    zN = get_zN(events.numpy(), n_discrete)
    right_term_precomp = term4(torch.tensor(zN), baseline, alpha, kernel, n_events)

    assert torch.isclose(right_term_conv, 2*right_term_precomp)
...
def test_l2loss():
    ...
    events, baseline, alpha, decay, discrete = set_data(end_time, 
                                                        n_discrete, 
                                                        random_state)
    n_events = events.sum(1)
...
def test_gradients():
     ...
     grad_kernel = model.get_grad([decay], discrete)
     grad_theta = get_grad_theta(torch.tensor(zG), 
                                 torch.tensor(zN),
                                 torch.tensor(ztzG),
                                 baseline, 
                                 adjacency, kernel, 
                                 grad_kernel[0], delta, n_events)
