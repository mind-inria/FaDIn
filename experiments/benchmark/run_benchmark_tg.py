...
def simulate_data(baseline, alpha, m, sigma, T, dt, seed=0):
    ...
    n_dim = m.shape[0]
    TG = DiscreteKernelFiniteSupport(dt, n_dim=n_dim, kernel='truncated_gaussian',
                                     lower=0, upper=1)
    ...
    intens = TG.intensity_eval(torch.tensor(baseline),
                               torch.tensor(alpha),
                               [torch.Tensor(m), torch.Tensor(sigma)],
...
def run_non_param_tick(events, size_grid, dt, seed=0):
    ...
    L = int(1 / dt)
    start = time.time()
    non_param = HawkesBasisKernels(1, n_basis=1, kernel_size=int(1 / dt), max_iter=100)
...
def run_experiment(baseline, alpha, m, sigma, T, dt, seed=0):
    ...
    size_grid = L * T + 1
    # simulate data
    events, intens = simulate_data(baseline, alpha, m, sigma, T, dt, seed=seed)
    events_grid = projected_grid(events, dt, size_grid)
    ...
    m_init = m + v
    sigma_init = sigma + v
    results = run_fadin(events, m_init, sigma_init, baseline_init,
                        alpha_init, T, dt, seed=0)
    baseline_hat = results['param_baseline']
    alpha_hat = results['param_alpha']
    m_hat = results['param_kernel'][0]
    sigma_hat = results['param_kernel'][1]

    TG = DiscreteKernelFiniteSupport(dt, n_dim=2, kernel='truncated_gaussian',
                                     lower=0, upper=1)
    intens_fadin = TG.intensity_eval(torch.tensor(baseline_hat),
...
T = 1000
dt = 0.01
# res = run_experiment(baseline, alpha, m, sigma, T, dt, seed=0)
# res
# %%
dt_list = [0.01]
...
df.to_csv('results/benchmark_tg.csv', index=False)

# %%

df = pd.read_csv('results/benchmark_tg.csv')
