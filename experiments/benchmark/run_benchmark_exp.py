...
def simulate_data(baseline, alpha, decay, T, dt, seed=0):
    ...
    intens = EXP.intensity_eval(torch.tensor(baseline),
                                torch.tensor(alpha),
                                [torch.Tensor(decay)],
...
def run_experiment(baseline, alpha, decay, T, dt, seed=0):
    ...
    size_grid = L * T + 1
    # simulate data
    events, intens = simulate_data(baseline, alpha, decay, T, dt, seed=seed)
    events_grid = projected_grid(events, dt, size_grid)
    ...
    results = run_fadin(events, decay_init, baseline_init,
...
baseline = np.array([1.1, 1.2])
alpha = np.array([[0.8, 0.1], [0.1, 0.8]])
decay = np.array([[0.5, 0.5], [0.5, 0.5]])


T = 1000
dt = 0.01
# res = run_experiment(baseline, alpha, decay, T, dt, seed=0)
# res
# %%
dt_list = [0.01]
...
df.to_csv('results/benchmark_exp.csv', index=False)

# %%

df = pd.read_csv('results/benchmark_exp.csv')
