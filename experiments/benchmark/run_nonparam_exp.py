...
def simulate_data(baseline, alpha, decay, T, dt, seed=0):
    ...
    RC = DiscreteKernelFiniteSupport(dt, 1, kernel='truncated_exponential')
    kernel_values = RC.kernel_eval([torch.Tensor(decay)],
                                   discretization)  # * dt
...
# events = simulate_data(baseline, alpha, decay, T, dt, seed=1)
...
def run_experiment(baseline, alpha, decay, T, dt, seed=0):
    ...
    decay_hd = results['param_kernel'][0][-1]
    alpha_hd = results['param_alpha'][-1]

    RC = DiscreteKernelFiniteSupport(dt, n_dim=1, kernel='truncated_exponential')
    kernel_values = RC.kernel_eval([torch.Tensor(decay_hd)],
                                   discretization).squeeze().numpy()
    kernel_values *= alpha_hd.item()

    res_our = dict(kernel=kernel_values, comp_time=time_our,
                   kernel_name='EXP', T=T, dt=dt, seed=seed)
