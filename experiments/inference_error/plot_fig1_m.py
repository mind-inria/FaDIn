...
def plot_fig1_paper(kernel='TG', leg=True):
    ...
    for m, ls, hatch in methods:
        for i, t in enumerate(T):
            ...
            curve = this_df.groupby("dt")["err_sum"].quantile(
                [0.25, 0.5, 0.75]).unstack()
            plt.loglog(
                curve.index, curve[0.5], ls, lw=4, c=palette[i],
                markersize=10, markevery=4 if is_EM else 3
            )
            plt.fill_between(
    ...
    plt.savefig(f"plots/approx/fig1_{kernel}_multi.png", bbox_inches='tight')
    plt.savefig(f"plots/approx/fig1_{kernel}_multi.pdf", bbox_inches='tight')
