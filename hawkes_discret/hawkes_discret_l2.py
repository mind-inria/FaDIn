...
class HawkesDiscretL2(object):
    ...
    def __init__(self, kernel_name, kernel_params, 
        ...
        self.discret_step = discret_step
        ...
        self.opt = optimizer(self.params_optim, lr=self.step_size, solver=self.solver)
    ...
    def lossl2(self):
        intensity = self.kernel_model.intensity_eval(self.baseline,
                                                     self.adjacency, 
                                                     self.events_loc_grid)

        return (((intensity**2).sum(1) * self.discret_step - 2 * (intensity*self.events_loc_grid_bool).sum(1))/self.end_time).sum()
    ...
    def fit(self, events, end_time):
        ...
        self.size_grid = int(end_time / self.discret_step)
        self.events = events
        self.end_time = end_time

        self.events_grid, self.events_loc_grid = projected_grid(self.events, 
                                                                self.discret_step, 
                                                                self.size_grid)
        ...
        for i in range(self.max_iter):
            ...
            self.kernel_params.grad = gradient_kernel() * self.kernel_model.grad_params()#grad chain rules


            self.opt.step() 

            if torch.isnan(self.baseline.grad).any() | torch.isnan(self.adjacency.grad).any() | torch.isnan(self.kernel_params.grad).any():
