    ...
    def eval(self, kernel_param, discretization):
        ...
        for i in range(n_dim):
            for j in range(n_dim):
                kernel_values[i, j] = torch.exp(
                    (
                        -torch.square(discretization - m[i, j])
        ...
        kernel_values[:, :, mask_kernel] = 0.0

        kernel_values /= kernel_values.sum(2)[:, :, None] * self.discrete_step
    ...
    def compute_grad(self, kernel_param, discretization):
        ...
        for i in range(n_dim):
            for j in range(n_dim):
                ...
                temp_mu = (
                    (discretization - m[i, j]) / (torch.square(sigma[i, j]))
                ) * temp

                temp_s = (
                    torch.square(discretization - m[i, j]) / (torch.pow(sigma[i, j], 3))
                ) * temp
                ...
                grad_m[i, j] = (temp_mu * temp_sum - temp * temp_mu_sum) / (
                    temp_sum ** 2
                )
...
class KernelExpDiscret(object):
    ...
    def eval(self, kernel_param, discretization):
        ...
        kernel_values = decay.unsqueeze(2) * torch.exp(
            -decay.unsqueeze(2) * discretization
        )
    ...
    def compute_grad(self, kernel_param, discretization):
        ...
        temp2 = (1 - decay.unsqueeze(2) * discretization) * torch.exp(
            -decay.unsqueeze(2) * discretization
        )
        temp1[:, :, 0] = 0.0
        temp2[:, :, 0] = 0.0
    ...
    def compute_grad_(self, kernel_param, discretization):
        ...
        """
        decay = kernel_param[0]
        kernel_grad = (1 - decay.unsqueeze(2) * discretization) * torch.exp(
            -decay.unsqueeze(2) * discretization
        )
    ...
    def intensity_eval(
        self, baseline, adjacency, kernel_param, events_grid, discretization
    ):
        ...
        for i in range(n_dim):
            intensity_temp[i, :, :] = torch.conv_transpose1d(
                ...
                kernel_values_adj[:, i].view(1, n_dim, self.size_discrete),
...
class KernelRaisedCosineDiscret(object):
    ...
    def eval(self, kernel_param, discretization):
        ...
        for i in range(n_dim):
            for j in range(n_dim):
                kernel[i, j] = 1 + torch.cos(
                    (discretization - u[i, j]) / sigma[i, j] * np.pi - np.pi
                )  # \
                # / (2 * sigma[i, j])
                mask_kernel = (discretization < u[i, j]) | (
                    discretization > (u[i, j] + 2 * sigma[i, j])
                )
                kernel[i, j, mask_kernel] = 0.0
    ...
    def compute_grad(self, kernel_param, discretization):
        ...
        for i in range(n_dim):
            for j in range(n_dim):
                ...
                grad_sigma[i, j] = (
                    np.pi * temp_1 / sigma[i, j]  # temp_3 à la place de sigma
                ) * torch.sin(
                    temp_2
                )  # - (1 +
                ...
                grad_u[i, j, mask_grad] = 0.0
                grad_sigma[i, j, mask_grad] = 0.0
