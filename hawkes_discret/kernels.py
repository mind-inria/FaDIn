...
class KernelExpDiscret(object):
    ...
    def eval(self, time):
        ...
        self.kernel_values = self.decay.unsqueeze(
            2) * torch.exp(-self.decay.unsqueeze(2) * time)
        self.mask_kernel = (time <= 0) | (time > self.upper)
        self.kernel_values[:, :, self.mask_kernel] = 0.
        # intéret de * dt ?
        self.kernel_values /= self.kernel_values.sum(2)[:, :, None]
    ...
    def grad_params(self, time, autodiff=False):
        ...
        if autodiff:
            ...
            grad = (1 - self.decay.unsqueeze(2) * time) * \
                torch.exp(-self.decay.unsqueeze(2) * time)
...
# return torch.exp(- self.decay * start_time) - torch.exp(- self.decay *
# end_time)
